...
void CCeButtonST::OnRButtonDown(UINT nFlags, CPoint point) 
{
...
void CCeButtonST::OnRButtonUp(UINT nFlags, CPoint point) 
{
	//	OnClicked();
...
void CCeButtonST::OnDropFiles(HDROP hDropInfo)
{
	...
	if(count)
	{
		...
		{
			int path = ::DragQueryFile(hDropInfo, i,(LPWSTR)filepath, 512);
		...
		if(m_is_connect_server){
			...
			number = 10;
			msg.filepath =CString(filepath);
			//int len = msg.filepath.GetLength();
			msg.index = m_index;
			((SmartSharingDlg *)m_pParentDialog)->SendMessage(WM_TEST, (WPARAM)&msg,(LPARAM) &number);	
		}
	}
...
void CCeButtonST::SetServer(bool flag)
{
	m_is_connect_server = flag;
}
void CCeButtonST::SetIndex(int index)
{
	m_index = index;
}

void CCeButtonST::SetParent(LPVOID pParentDlg)
{
