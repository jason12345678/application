...
#define BS_TYPEMASK1 0x0000001FL		//SS_TYPEMASK
...
void CCeButtonST1::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CCeButtonST1::PreSubclassWindow() 
{
	UINT nBS;
...
UINT CCeButtonST1::OnGetDlgCode() 
{
...
LRESULT CCeButtonST1::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
HBRUSH CCeButtonST1::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

LRESULT CCeButtonST1::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
	UINT nNewType = (wParam & BS_TYPEMASK1);
...
void CCeButtonST1::OnEnable(BOOL bEnable) 
{
...
void CCeButtonST1::OnKillFocus(CWnd* pNewWnd) 
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CCeButtonST1::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CCeButtonST1::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

void CCeButtonST1::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CCeButtonST1::OnRButtonDown(UINT nFlags, CPoint point) 
{
...
void CCeButtonST1::OnRButtonUp(UINT nFlags, CPoint point) 
{
	//	OnClicked();
	
}
void CCeButtonST1::CancelHover()
{
...
void CCeButtonST1::OnMouseMove(UINT nFlags, CPoint point) 
{
...
LRESULT CCeButtonST1::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CCeButtonST1::OnClicked() 
{
	if (m_bIsCheckBox)
	{
		m_nCheck = !m_nCheck;
...
void CCeButtonST1::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
...
//
void CCeButtonST1::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CCeButtonST1::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
//
DWORD CCeButtonST1::SetIcon(int nIconIn, CSize sizeIn, int nIconOut, CSize sizeOut, int nIconDis, CSize sizeDis)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
	HICON		hIconDis		= NULL;
...
//			Function executed successfully.
//
DWORD CCeButtonST1::SetIcon(HICON hIconIn, CSize sizeIn, HICON hIconOut, CSize sizeOut, HICON hIconDis, CSize sizeDis)
{
...
//			Function executed successfully.
//
DWORD CCeButtonST1::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//
DWORD CCeButtonST1::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//			Function executed successfully.
//
DWORD CCeButtonST1::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CCeButtonST1::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CCeButtonST1::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CCeButtonST1::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//			Function executed successfully.
//
DWORD CCeButtonST1::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//
DWORD CCeButtonST1::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
	// Restore old cursor (if any)
	if (m_hOldCursor)	::SetCursor(m_hOldCursor);
...
//			Function executed successfully.
//
DWORD CCeButtonST1::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CCeButtonST1::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CCeButtonST1::SetURL(LPCTSTR lpszURL)
{
...
BOOL CCeButtonST1::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	HCURSOR	hOldCursor;
...
//			Failed loading the specified resource.
//
DWORD CCeButtonST1::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
//			Function executed successfully.
//
DWORD CCeButtonST1::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CCeButtonST1::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	if (m_bIsPressed)
...
void CALLBACK CCeButtonST1::TimerProc(HWND hwnd, UINT uMsg, UINT idEvent, DWORD dwTime)
{
	POINT	csPos;
	RECT	csRect;
...
void CCeButtonST1::OnDropFiles(HDROP hDropInfo)
{
	...
	if(count)
	{
		...
		int number;
		dragmsg msg;
		if(m_is_connect_server){

			number = 9;
		
			msg.filepath = CString((LPWSTR)filePath);
			msg.index = 0;
			((SmartSharingDlg *)m_pParentDialog)->SendMessage(WM_TEST, (WPARAM)&msg,(LPARAM) &number);	
		}else{

			number = 10;
			msg.filepath = CString(filePath);
			msg.index = m_index;
			((SmartSharingDlg *)m_pParentDialog)->SendMessage(WM_TEST, (WPARAM)&msg,(LPARAM) &number);	
		}
	}
...
void CCeButtonST1::SetServer(bool flag)
{
	m_is_connect_server = flag;
}
void CCeButtonST1::SetIndex(int index)
{
	m_index = index;
}

void CCeButtonST1::SetParent(LPVOID pParentDlg)
{
