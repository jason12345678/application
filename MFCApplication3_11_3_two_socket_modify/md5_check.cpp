...
char* MD5CheckFile(char *output,char *filepath )
{
	...
	while ((nLength = fread( Buffer,sizeof(BYTE),nBufferSize,fp )) > 0 )
	{
		...
		MD5Update( &context, Buffer, nLength ); 
	}
...
*/ 
int MD5Check( char *md5string, char* string,unsigned int len ) 
{ 
	char tmp[100];
	memset(tmp,0,sizeof(tmp));
	MD5String(tmp,string,len);
return strncmp( md5string, tmp,32) == 0; 
} 
